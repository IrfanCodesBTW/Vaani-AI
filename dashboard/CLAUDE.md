# Vaani AI. Repo guide for future sessions.
...
A premium, multi-tenant, provider-agnostic AI voice agent platform, branded **Vaani AI**.
...
```sh
cd ~/iCloud/website/vaani-ai-console   # real path: ~/Library/Mobile Documents/com~apple~CloudDocs/website/vaani-ai-console
...
vaani-ai-console/
  ...
  public/
    ...
    assets/
      ...
      logo.svg              # Vaani AI wordmark with voltage waveform glyph (Ops)
  ...
  package.json              # name vaani-ai, "start": "node server.js", no dependencies
  .gitignore
  README.md
  CLAUDE.md                 # this file
...
- **Passwords**: `crypto.scryptSync` with a random salt, stored as `scrypt$<saltHex>$<hashHex>`.
  Never plaintext.
- **Sessions**: opaque `crypto.randomBytes(32)` hex token, httpOnly cookie `vaani_sess`, 7 day expiry.
...
## API contract summary (all JSON unless noted, auth via `vaani_sess` cookie)
