...
All endpoints use the existing `vaani_sess` HttpOnly cookie. Every customer resource is scoped from the authenticated session tenant. IDs in request bodies never select tenant scope. Money is represented as integer paise and wallets use INR.
