# Vaani AI , Build Contract (single source of truth for the swarm)
...
**Vaani AI**. The hook: production voice agents at roughly one rupee, powered by
...
```
{
  ...
  "users":   [ { "id":"u_xxx", "tenantId":"t_xxx", "email":"a@b.com",
                 "name":"Irfan", "passHash":"scrypt$...", "role":"owner", "createdAt":ISO } ],
...
- Sessions: opaque random token (`crypto.randomBytes(32).hex`), httpOnly cookie `vaani_sess`, 7 day expiry.
...
- On boot: if `data/db.json` missing, create it and seed a demo tenant + owner
  (email `demo@vaani.ai`, password `vaaniai`, tenant `Vaani Demo`) and migrate any
  legacy `agents.json` agents into that tenant. Print the demo login to the console.

## 4. API contract (all JSON unless noted). Auth via `vaani_sess` cookie.
...
## 7. Marketing landing (`index.html`) , follow the marketing landing formula EXACTLY
