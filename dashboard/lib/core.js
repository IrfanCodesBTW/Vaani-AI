/**
 * Vaani AI. Core runtime primitives. ZERO npm dependencies.
 *
 ...
 *   - auth: scrypt password hashing, opaque session tokens, the vaani_sess cookie,
...
function httpsPost(host, pathname, headers, bodyBuf) {
  return httpsRequest(host, pathname, 'POST', headers, bodyBuf);
}

// Generic HTTPS request with optional body. Times out at 60s.
function httpsRequest(host, pathname, method, headers, bodyBuf) {
  return new Promise((resolve, reject) => {
...
function defaultDb() {
  return {
    ...
    memberships: [], invitations: [], providerCredentials: [], callRuns: [], campaigns: [],
    callTranscripts: [],
  };
}
...
const COLLECTIONS = [
  ...
  'invoices', 'invoiceEvents', 'integrationRequests', 'agencyPrompts',
  'clientActivities', 'tenantStatusEvents', 'memberships', 'invitations',
  'providerCredentials', 'callRuns', 'campaigns', 'callTranscripts',
];
...
function migrateDb(parsed) {
  ...
  if (out.schemaVersion < 5) {
    ...
    saas.migrateTenantMemberships(out);
    out.schemaVersion = 5;
  }
  if (out.schemaVersion < 6) {
    if (!Array.isArray(out.callTranscripts)) out.callTranscripts = [];
    for (const tenant of out.tenants) {
      if (tenant.defaultOutboundAgentId === undefined) tenant.defaultOutboundAgentId = null;
    }
    out.schemaVersion = 6;
  }
  return out;
...
async function getSession(req) {
  ...
  const saas = require('./saas');
  const membership = saas.resolveMembership(d, user, tenant.id);
  if (!membership && !saas.isPlatformUser(user)) return null;
  return { session, user, tenant, impersonator, membership };
}
 ...
 */
let tenantContextRunner = null;

function setTenantContextRunner(fn) {
  tenantContextRunner = typeof fn === 'function' ? fn : null;
}
...
async function requireAuth(req, res, handler, body) {
  ...
  const run = () => handler(req, res, { ...ctx, body });
  if (tenantContextRunner && ctx.tenant && ctx.tenant.id) {
    return tenantContextRunner(ctx.tenant.id, run);
  }
  return run();
}
...
module.exports = {
  ...
  loadEnv,
  send, sendJson, readBody, httpsPost, httpsGet, httpsRequest,
  htmlEscape,
  db, mutate, loadDb, defaultDb, migrateDb,
  hashPassword, verifyPassword,
  createSession, createImpersonationSession, destroySession, getSession, requireAuth, requireRole, hasRole, parseCookieToken,
  setTenantContextRunner,
