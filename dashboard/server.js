/**
 * Vaani AI. Zero-dependency Node server (the product, multi-tenant).
 *
...
core.setTenantContextRunner(credentials.runWithTenant);
const payu = require('./lib/payu');
const demoLinks = require('./lib/demo-links');
const saas = require('./lib/saas');
const transcripts = require('./lib/call-transcripts');
const agentWorkflow = require('./lib/agent-workflow');
...
const DEMO_TENANT = String(process.env.TEST_USER_TENANT || 'Vaani Test');
const TRIAL_CREDIT_PAISE = 1000;
const CREDIT_PACKS = Object.freeze({
  starter: Object.freeze({ amount: '200.00', currency: 'INR', credits: 20000, productinfo: 'Vaani AI Starter Credits' }),
  growth: Object.freeze({ amount: '500.00', currency: 'INR', credits: 50000, productinfo: 'Vaani AI Growth Credits' }),
  scale: Object.freeze({ amount: '1000.00', currency: 'INR', credits: 100000, productinfo: 'Vaani AI Scale Credits' }),
});
...
async function boot() {
  ...
  if (DEMO_EMAIL && DEMO_PASS.length >= 12 && !hasDemo) {
    ...
    await core.mutate((d) => {
      ...
      saas.ensureMembership(d, userId, tenantId, 'owner');
  ...
  if (core.db().tenants.some((t) => !t.pipeline || !t.providers || !t.providers.stt || !t.providers.tts || !t.providers.llm || !t.providers.telephony)) {
    await core.mutate((d) => {
      d.tenants.forEach((t) => {
        t.providers = { ...DEFAULT_PROVIDERS, ...(t.providers || {}) };
        if (!t.pipeline) {
          t.pipeline = providers.publicPipeline({
            stt: { provider: t.providers.stt },
            llm: { provider: t.providers.llm },
            tts: { provider: t.providers.tts },
          });
        }
      });
    });
...
function tenantPipeline(tenant) {
  const stored = (tenant && tenant.pipeline) || {};
  if (!tenant.pipeline && tenant.providers) {
    return providers.mergePipeline({
      stt: { provider: tenant.providers.stt },
      llm: { provider: tenant.providers.llm },
      tts: { provider: tenant.providers.tts },
    });
  }
  return providers.mergePipeline(stored);
}

function agentPipeline(tenant, agent) {
  const base = tenantPipeline(tenant);
  if (!agent || agent.useWorkspacePipeline !== false) return base;
  const override = agent.pipeline || {};
  return {
    stt: override.stt ? providers.normalizePipelineLayer('stt', override.stt) : base.stt,
    llm: override.llm ? providers.normalizePipelineLayer('llm', override.llm) : base.llm,
    tts: override.tts ? providers.normalizePipelineLayer('tts', override.tts) : base.tts,
  };
}

function resolveLayerSelection(tenant, agent, layer) {
  const pipeline = agent ? agentPipeline(tenant, agent) : tenantPipeline(tenant);
  return pipeline[layer];
}

function publicTenant(t) {
  return {
    ...saas.publicOrganization(t),
    providers: t.providers,
    pipeline: providers.publicPipeline(t.pipeline || {}),
  };
}
...
function publicAgent(a) {
  return {
    ...
    useWorkspacePipeline: a.useWorkspacePipeline !== false,
    pipeline: a.pipeline ? providers.publicPipeline(a.pipeline) : null,
    dograhWorkflowId: a.dograhWorkflowId || null,
    restricted: !!a.restricted,
    capabilities: transcripts.agentCapabilities(a),
    createdAt: a.createdAt,
  };
}

function orgMembership(ctx) {
  return ctx.membership || saas.resolveMembership(core.db(), ctx.user, ctx.tenant.id);
}

async function persistAgentWorkflowId(tenantId, agentId, workflowId) {
  await core.mutate((d) => {
    const row = d.agents.find((item) => item.id === agentId && item.tenantId === tenantId);
    if (row) row.dograhWorkflowId = workflowId;
  });
}

async function publishAgentToDograh(agent, tenantId) {
  const workflowId = await agentWorkflow.syncAgentWorkflow(agent);
  if (tenantId && agent && agent.id) await persistAgentWorkflowId(tenantId, agent.id, workflowId);
  agent.dograhWorkflowId = workflowId;
  return workflowId;
}

function createPendingTranscript(d, tenant, callRun, agent, extras) {
  const privacyBlocked = tenant.privacyMode === 'no_recording';
  ...
  const row = {
    ...
    destination: extras.destination || callRun.destination || '',
    provider: callRun.provider,
    workflowId: extras.workflowId || callRun.workflowId,
    workflowRunId: extras.workflowRunId || callRun.workflowRunId || null,
    workflowRunName: extras.workflowRunName || callRun.workflowRunName || '',
    ...
    summary: privacyBlocked
      ? transcripts.emptySummary('Transcript retention is disabled for this workspace.')
      : transcripts.emptySummary('Waiting for the live call to finish.'),
    searchText: '',
    status: privacyBlocked ? 'blocked_privacy' : 'pending',
    createdAt: now,
    updatedAt: now,
  };
  d.callTranscripts.push(row);
  return row;
}

function transcriptNeedsRefresh(row) {
  if (!row || row.status === 'blocked_privacy') return false;
  if (row.status === 'pending') return true;
  return row.status === 'unavailable' && !(row.turns || []).length && !String(row.verbatim || '').trim();
}

async function refreshTranscriptRow(row, tenant) {
  if (!row || row.status === 'blocked_privacy') return row;
  if (!row.workflowId) {
    row.status = 'unavailable';
    row.updatedAt = new Date().toISOString();
    return row;
  }
  const run = await providers.resolveWorkflowRun(row.workflowId, {
    runId: row.workflowRunId,
    runName: row.workflowRunName,
    sinceIso: row.startedAt,
  });
  if (!run) return row;
  transcripts.applyDograhRun(row, run);
  if (row.status === 'ready' && (!row.summary || row.summary.overview === 'Waiting for the live call to finish.')) {
    row.summary = await transcripts.summarizeTranscript(providers.llm, row.verbatim, row.agentName);
    row.searchText = transcripts.searchBlob(row);
  }
  await core.mutate((d) => {
    const current = d.callTranscripts.find((item) => item.id === row.id && item.tenantId === tenant.id);
    if (!current) return;
    ...
    const call = d.callRuns.find((item) => item.id === row.callRunId && item.tenantId === tenant.id);
    if (call) {
      call.workflowRunId = row.workflowRunId;
      call.workflowRunName = row.workflowRunName;
      call.durationSec = row.durationSec || call.durationSec;
      if (row.status === 'ready' || row.status === 'unavailable') {
        call.status = row.status === 'unavailable' ? call.status : 'completed';
        call.endedAt = row.endedAt || call.endedAt;
      }
    }
  });
  return row;
...
async function apiSignup(req, res, body) {
  ...
  await core.mutate((d) => {
    ...
    tenant = {
      ...
      providers: { ...DEFAULT_PROVIDERS },
      plan: 'studio',
      status: 'active', privacyMode: 'standard', parent_account_id: null,
    };
    user = {
  ...
  core.send(res, 200, JSON.stringify({ user: publicUser(user, { role: 'owner' }), tenant: publicTenant(tenant) }), {
...
async function apiLogin(req, res, body) {
  ...
  const membership = saas.findMembership(d, user.id, tenant.id);
  await core.mutate((store) => addAudit(store, { tenant, user, membership }, 'auth.login', 'user', user.id));
  core.send(res, 200, JSON.stringify({ user: publicUser(user, membership), tenant: publicTenant(tenant) }), {
    'Content-Type': 'application/json',
    'Set-Cookie': core.sessionCookie(token),
  });
}
...
function apiMe(req, res, ctx) {
  const organizations = saas.listOrganizationsForUser(core.db(), ctx.user);
  core.sendJson(res, 200, {
    user: publicUser(ctx.user, ctx.membership),
    tenant: publicTenant(ctx.tenant),
    organizations,
    membership: ctx.membership ? saas.publicMembership(ctx.membership, ctx.tenant) : null,
    impersonation: ctx.impersonator ? { actor: publicUser(ctx.impersonator), reason: ctx.session.impersonationReason, expiresAt: new Date(ctx.session.exp).toISOString() } : null,
  });
}
...
async function apiAgentsCreate(req, res, ctx) {
  ...
  const ttsIn = b.tts || {};
  const workspace = tenantPipeline(ctx.tenant);
  const useWorkspacePipeline = b.useWorkspacePipeline !== false;
  const pipeline = useWorkspacePipeline ? null : providers.publicPipeline(b.pipeline || {});
  const ttsLayer = useWorkspacePipeline ? workspace.tts : (pipeline ? providers.normalizePipelineLayer('tts', pipeline.tts || {}) : workspace.tts);
  const ttsProvider = ['rumik', 'sarvam'].includes(String(ttsIn.provider || ttsLayer.provider || 'rumik').toLowerCase())
    ? String(ttsIn.provider || ttsLayer.provider || 'rumik').toLowerCase()
    : 'rumik';
  const defaultModel = ttsProvider === 'sarvam' ? 'bulbul:v3' : 'mulberry';
  const model = ttsIn.model || ttsLayer.model || defaultModel;
  const speaker = providers.TTS_SPEAKERS.has(ttsIn.speaker) ? ttsIn.speaker : (ttsLayer.speaker || 'speaker_1');
  const f0 = Number.isFinite(ttsIn.f0_up_key) ? Math.max(-12, Math.min(12, ttsIn.f0_up_key | 0)) : 0;
  const voice = String(ttsIn.voice || ttsLayer.voice || 'shubh').slice(0, 64);
  const description = String(ttsIn.description || '').slice(0, 500);

  const agent = {
    id: core.genId('ag_'),
    tenantId: ctx.tenant.id,
    name: String(b.name || (preset && preset.name) || 'Untitled Agent').slice(0, 60),
    persona: String(b.persona || (preset ? `${preset.name}. Collect: ${preset.fields.join(', ')}. Guardrails: ${preset.guardrails.join('; ')}.` : '')).slice(0, 8000),
    tts: { provider: ttsProvider, model, speaker, f0_up_key: f0, voice, description },
    ...
    dograhWorkflowId: Number.isInteger(Number(b.dograhWorkflowId)) && Number(b.dograhWorkflowId) > 0 ? Number(b.dograhWorkflowId) : null,
    restricted: b.restricted === true,
    createdAt: new Date().toISOString(),
  };
  await core.mutate((d) => { d.agents.push(agent); });
  try { await publishAgentToDograh(agent, ctx.tenant.id); } catch (e) {
...
async function apiAgentsUpdate(req, res, ctx) {
  ...
  await core.mutate((dd) => {
    ...
    if (b.persona != null) a.persona = String(b.persona).slice(0, 8000);
    if (b.greeting != null) a.greeting = String(b.greeting).slice(0, 600);
    if (b.did != null) {
      const did = String(b.did).replace(/[^0-9]/g, '');
      a.telephony = { ...(a.telephony || {}), did: did || providers.telephony.did };
    }
    if (b.dograhWorkflowId != null) {
      const wf = Number(b.dograhWorkflowId);
      a.dograhWorkflowId = Number.isInteger(wf) && wf > 0 ? wf : null;
    }
    if (b.restricted != null) a.restricted = b.restricted === true;
    if (b.useWorkspacePipeline != null) a.useWorkspacePipeline = b.useWorkspacePipeline !== false;
    if (b.pipeline && typeof b.pipeline === 'object') {
      a.pipeline = providers.publicPipeline(b.pipeline);
      a.useWorkspacePipeline = false;
    }
    if (b.tts && typeof b.tts === 'object') {
      const layer = agentPipeline(ctx.tenant, a).tts;
      const t = a.tts || { provider: layer.provider };
      if (b.tts.provider != null && ['rumik', 'sarvam'].includes(String(b.tts.provider).toLowerCase())) {
        t.provider = String(b.tts.provider).toLowerCase();
      }
      if (b.tts.model != null) t.model = b.tts.model;
      ...
      if (b.tts.voice) t.voice = String(b.tts.voice).slice(0, 64);
      if (b.tts.description != null) t.description = String(b.tts.description).slice(0, 500);
      if (!t.provider) t.provider = layer.provider;
      a.tts = t;
    }
    updated = a;
  });
  try { await publishAgentToDograh(updated, ctx.tenant.id); } catch (e) {
    return handleProviderError(res, e);
  }
...
async function apiTts(req, res, ctx) {
  const b = ctx.body || {};
  try {
    const layer = resolveLayerSelection(ctx.tenant, null, 'tts');
    const selected = providers.resolveSelection('tts', {
      provider: b.provider || layer.provider,
      model: b.model || layer.model,
      speaker: b.speaker || layer.speaker,
      voice: b.voice || layer.voice,
      language: b.language || layer.language,
      description: b.description,
    });
    const out = await selected.adapter.synthesize({
      text: b.text,
      model: selected.model,
      speaker: selected.speaker || b.speaker,
      voice: selected.voice || b.voice,
      language: selected.language,
      f0_up_key: b.f0_up_key,
      description: selected.description || b.description,
    });
...
async function apiWsConnect(req, res, ctx) {
  ...
  try {
    const layer = resolveLayerSelection(ctx.tenant, null, 'tts');
    const selected = providers.resolveSelection('tts', { provider: b.provider || layer.provider, model: b.model || layer.model });
...
async function apiChat(req, res, ctx) {
  const b = ctx.body || {};
  try {
    const layer = resolveLayerSelection(ctx.tenant, null, 'llm');
    const selected = providers.resolveSelection('llm', { provider: b.provider || layer.provider, model: b.model || layer.model });
...
async function apiStt(req, res, ctx) {
  const b = ctx.body || {};
  try {
    const layer = resolveLayerSelection(ctx.tenant, null, 'stt');
    const selected = providers.resolveSelection('stt', {
      provider: b.provider || layer.provider,
      model: b.model || layer.model,
      language: b.language || layer.language,
    });
    const out = await selected.adapter.transcribe({ audio: b.audio, mime: b.mime, model: selected.model, language: selected.language });
    core.sendJson(res, 200, out);
  } catch (e) {
    handleProviderError(res, e);
  }
...
async function apiVoiceSession(req, res, ctx) {
  try {
    ...
    await core.mutate((d) => {
      saas.recordCallRun(d, ctx.tenant.id, {
        agentId: String((ctx.body || {}).agentId || '') || null,
        direction: 'inbound',
        provider: 'webrtc',
        status: 'completed',
        outcome: 'answered',
...
// GET /api/telephony/status -> VoBiz and VoiceLink carrier status.
async function apiTelephonyStatus(req, res) {
  const q = new URL(req.url || '/', 'http://localhost').searchParams;
  const only = String(q.get('provider') || '').trim().toLowerCase();
  const wrap = async (id) => {
    try {
      const status = await providers.get('telephony', id).status();
      return { ok: true, ...status, provider: id };
    } catch (e) {
      return {
        ok: false,
        provider: id,
        connected: false,
        error: e instanceof providers.ProviderError ? e.message : String(e.message || e),
        code: e instanceof providers.ProviderError ? e.code : 'upstream',
        detail: e instanceof providers.ProviderError ? e.detail : undefined,
      };
    }
  };
  try {
    if (only === 'vobiz' || only === 'voicelink') {
      return core.sendJson(res, 200, await wrap(only));
    }
    const [vobiz, voicelink] = await Promise.all([wrap('vobiz'), wrap('voicelink')]);
    core.sendJson(res, 200, { carriers: { vobiz, voicelink }, provider: 'multi' });
  } catch (e) {
    handleProviderError(res, e);
  }
}
...
async function apiTelephonyDial(req, res, ctx) {
  ...
  const membership = orgMembership(ctx);
  if (!transcripts.canPlaceOutbound(membership, ctx.user)) {
    return core.sendJson(res, 403, { error: 'outbound calling requires an operator role', code: 'forbidden' });
  }
  const agent = core.db().agents.find((a) => a.id === String(b.agentId || '') && a.tenantId === ctx.tenant.id);
  if (!agent) {
    return core.sendJson(res, 422, { error: 'select a voice agent before placing the call', code: 'agent_required' });
  }
  if (!transcripts.canSelectAgent(agent, membership, ctx.user)) {
    return core.sendJson(res, 403, { error: 'this agent type requires an admin role', code: 'restricted_agent' });
  }
  let workflowId;
  try {
    workflowId = await publishAgentToDograh(agent, ctx.tenant.id);
  } catch (e) {
    return handleProviderError(res, e);
  }
  ...
  if (!Number.isInteger(workflowId) || workflowId <= 0) {
    return core.sendJson(res, 409, {
      error: 'this agent has no Dograh workflow ID. Set one on the agent or configure DOGRAH_WORKFLOW_ID.',
      code: 'workflow_required',
    });
  }
  try {
    const carrier = String(b.provider || b.carrier || ctx.tenant.providers?.telephony || 'vobiz').trim().toLowerCase();
    const adapter = providers.get('telephony', carrier);
    const r = await adapter.dial(b.number, { workflowId });
    const parsed = transcripts.parseInitiateCallMessage(r.data);
    const destination = '+91' + String(b.number || '').replace(/\D/g, '').slice(-10);
    bumpUsage(ctx.tenant.id, 'calls', 1).catch(() => {});
    let callRun;
    let transcript;
    await core.mutate((d) => {
      callRun = saas.recordCallRun(d, ctx.tenant.id, {
        agentId: agent.id,
        campaignId: String(b.campaignId || '') || null,
        direction: 'outbound',
        provider: carrier,
        status: r.status >= 200 && r.status < 300 ? 'initiated' : 'failed',
        outcome: r.status >= 200 && r.status < 300 ? 'answered' : 'failed',
        durationSec: 0,
        aiSpendPaise: 90,
        destination,
        workflowId,
        workflowRunId: parsed.workflowRunId,
        workflowRunName: parsed.workflowRunName,
        metadata: { dialResponseStatus: r.status, agentName: agent.name },
      });
      transcript = createPendingTranscript(d, ctx.tenant, callRun, agent, {
        destination,
        workflowId,
        workflowRunId: parsed.workflowRunId,
        workflowRunName: parsed.workflowRunName,
      });
      addAudit(d, ctx, 'telephony.dial', 'telephony', destination.slice(-4), {
        status: r.status, agentId: agent.id, workflowId,
      });
    });
    if (!parsed.workflowRunId && parsed.workflowRunName) {
      try {
        const run = await providers.resolveWorkflowRun(workflowId, {
          runName: parsed.workflowRunName,
          sinceIso: callRun.startedAt,
        });
        if (run && run.id) {
          parsed.workflowRunId = run.id;
          await core.mutate((d) => {
            const cr = d.callRuns.find((item) => item.id === callRun.id && item.tenantId === ctx.tenant.id);
            const tx = d.callTranscripts.find((item) => item.id === transcript.id && item.tenantId === ctx.tenant.id);
            if (cr) cr.workflowRunId = run.id;
            if (tx) tx.workflowRunId = run.id;
          });
          callRun.workflowRunId = run.id;
          transcript.workflowRunId = run.id;
        }
      } catch (_) { /* run id resolves on first transcript refresh */ }
    }
    core.sendJson(res, r.status, {
      ...r.data,
      call: {
        id: callRun.id,
        agentId: agent.id,
      ...
      transcript: transcripts.publicTranscript(transcript, [agent]),
    });
  } catch (e) {
    handleProviderError(res, e);
  }
...
function apiTelephonyDefaultAgentGet(req, res, ctx) {
  core.sendJson(res, 200, { agentId: ctx.tenant.defaultOutboundAgentId || null });
}

async function apiTelephonyDefaultAgentSet(req, res, ctx) {
  if (rejectImpersonated(res, ctx)) return;
  const membership = orgMembership(ctx);
  if (!transcripts.canSetDefaultAgent(membership, ctx.user)) {
    return core.sendJson(res, 403, { error: 'only admins can set the workspace default agent', code: 'forbidden' });
  }
  ...
  let tenant;
  await core.mutate((d) => {
    tenant = d.tenants.find((row) => row.id === ctx.tenant.id);
    tenant.defaultOutboundAgentId = agent.id;
    addAudit(d, ctx, 'telephony.default_agent.updated', 'agent', agent.id);
  });
  core.sendJson(res, 200, { agentId: tenant.defaultOutboundAgentId, tenant: publicTenant(tenant) });
}

function apiTelephonyTranscriptsList(req, res, ctx) {
  const membership = orgMembership(ctx);
  if (!transcripts.canViewTranscripts(membership, ctx.user)) {
    return core.sendJson(res, 403, { error: 'transcripts require an analyst role', code: 'forbidden' });
  }
  apiTelephonyTranscriptsListImpl(req, res, ctx).catch((e) => handleProviderError(res, e));
}

async function apiTelephonyTranscriptsListImpl(req, res, ctx) {
  const q = new URL(req.url || '/', 'http://localhost').searchParams;
  const filterArgs = {
    q: q.get('q') || '',
    agentId: q.get('agentId') || '',
    destination: q.get('number') || q.get('destination') || '',
    from: q.get('from') || '',
    to: q.get('to') || '',
  };
  let rows = core.db().callTranscripts.filter((row) => row.tenantId === ctx.tenant.id);
  const pending = transcripts.filterTranscripts(rows, filterArgs)
    .filter((row) => transcriptNeedsRefresh(row))
    .slice(0, 8);
  for (const row of pending) {
    try { await refreshTranscriptRow({ ...row }, ctx.tenant); } catch (_) { /* keep cached row */ }
  }
  rows = core.db().callTranscripts.filter((row) => row.tenantId === ctx.tenant.id);
  const filtered = transcripts.filterTranscripts(rows, filterArgs)
    .sort((a, b) => String(b.startedAt).localeCompare(String(a.startedAt)));
  const agents = core.db().agents.filter((a) => a.tenantId === ctx.tenant.id);
  core.sendJson(res, 200, {
    transcripts: filtered.slice(0, 100).map((row) => transcripts.publicTranscript(row, agents)),
    defaultAgentId: ctx.tenant.defaultOutboundAgentId || null,
  });
}

async function apiTelephonyTranscriptGet(req, res, ctx, id) {
  const membership = orgMembership(ctx);
  if (!transcripts.canViewTranscripts(membership, ctx.user)) {
    return core.sendJson(res, 403, { error: 'transcripts require an analyst role', code: 'forbidden' });
  }
  const row = core.db().callTranscripts.find((item) => item.id === id && item.tenantId === ctx.tenant.id);
  if (!row) return core.sendJson(res, 404, { error: 'transcript not found', code: 'not_found' });
  let current = row;
  if (transcriptNeedsRefresh(row)) {
    try { current = await refreshTranscriptRow({ ...row }, ctx.tenant); } catch (_) { /* keep cached row */ }
  }
  const agents = core.db().agents.filter((a) => a.tenantId === ctx.tenant.id);
  core.sendJson(res, 200, { transcript: transcripts.publicTranscript(current, agents) });
}

async function apiTelephonyTranscriptRefresh(req, res, ctx, id) {
  const membership = orgMembership(ctx);
  if (!transcripts.canViewTranscripts(membership, ctx.user)) {
    return core.sendJson(res, 403, { error: 'transcripts require an analyst role', code: 'forbidden' });
  }
  const row = core.db().callTranscripts.find((item) => item.id === id && item.tenantId === ctx.tenant.id);
  if (!row) return core.sendJson(res, 404, { error: 'transcript not found', code: 'not_found' });
  if (row.status === 'blocked_privacy') {
    return core.sendJson(res, 409, { error: 'transcripts are disabled in HIPAA mode', code: 'privacy_blocked' });
  }
  try {
    const current = await refreshTranscriptRow({ ...row }, ctx.tenant);
    const agents = core.db().agents.filter((a) => a.tenantId === ctx.tenant.id);
    core.sendJson(res, 200, { transcript: transcripts.publicTranscript(current, agents) });
  } catch (e) {
    handleProviderError(res, e);
...
function apiTelephonyAgentAnalytics(req, res, ctx) {
  const membership = orgMembership(ctx);
  if (!transcripts.canViewTranscripts(membership, ctx.user)) {
    return core.sendJson(res, 403, { error: 'agent analytics require an analyst role', code: 'forbidden' });
  }
  core.sendJson(res, 200, { agents: transcripts.agentPerformance(core.db(), ctx.tenant.id) });
}
...
function apiMembers(req, res, ctx) {
  const d = core.db();
  const memberships = d.memberships.filter((row) => row.organizationId === ctx.tenant.id && row.status === 'active');
  const users = memberships.map((membership) => {
    const user = d.users.find((row) => row.id === membership.userId);
    if (!user) return null;
    return { ...publicUser(user, membership), membershipId: membership.id, orgRole: membership.role };
  }).filter(Boolean);
  core.sendJson(res, 200, { users });
}
...
async function apiTenantPipeline(req, res, ctx) {
  if (rejectImpersonated(res, ctx)) return;
  const b = ctx.body || {};
  let tenant;
  try {
    const next = providers.publicPipeline({
      stt: b.stt || {},
      llm: b.llm || {},
      tts: b.tts || {},
    });
    await core.mutate((store) => {
      tenant = store.tenants.find((row) => row.id === ctx.tenant.id);
      tenant.pipeline = next;
      tenant.providers = {
        ...(tenant.providers || {}),
        stt: next.stt.provider,
        llm: next.llm.provider,
        tts: next.tts.provider,
      };
      addAudit(store, ctx, 'tenant.pipeline.updated', 'tenant', tenant.id, next);
    });
    core.sendJson(res, 200, { tenant: publicTenant(tenant), pipeline: next });
  } catch (e) {
    handleProviderError(res, e);
  }
}
...
async function apiMemberRole(req, res, ctx) {
  ...
  if (!saas.ORG_ROLES.includes(role)) return core.sendJson(res, 422, { error: 'organization roles are owner, admin, operator, analyst, or viewer', code: 'bad_role' });
  let membership = core.db().memberships.find((row) => row.id === String(b.membershipId || '') && row.organizationId === ctx.tenant.id);
  if (!membership && b.userId) {
    membership = core.db().memberships.find((row) => row.userId === String(b.userId) && row.organizationId === ctx.tenant.id);
  }
  if (!membership) return core.sendJson(res, 404, { error: 'membership not found', code: 'not_found' });
  await core.mutate((d) => {
    const target = d.memberships.find((row) => row.id === membership.id);
    target.role = role;
    const user = d.users.find((row) => row.id === target.userId);
    if (user && user.tenantId === ctx.tenant.id && role === 'owner') user.role = 'owner';
    addAudit(d, ctx, 'member.role.updated', 'membership', target.id, { role, userId: target.userId });
  });
  core.sendJson(res, 200, { membership: { id: membership.id, role, userId: membership.userId } });
}

function apiOrganizations(req, res, ctx) {
  core.sendJson(res, 200, { organizations: saas.listOrganizationsForUser(core.db(), ctx.user), activeOrganizationId: ctx.tenant.id });
}

async function apiOrganizationSwitch(req, res, ctx) {
  if (rejectImpersonated(res, ctx)) return;
  const organizationId = String((ctx.body || {}).organizationId || '');
  if (!organizationId) return core.sendJson(res, 422, { error: 'organizationId required', code: 'bad_organization' });
  const switched = await saas.switchOrganization(req, ctx.user, organizationId);
  if (switched.error) return core.sendJson(res, switched.status || 400, { error: switched.error, code: switched.code });
  const membership = saas.resolveMembership(core.db(), ctx.user, organizationId);
  await core.mutate((d) => addAudit(d, { ...ctx, tenant: switched.tenant, membership }, 'organization.switched', 'organization', organizationId));
  core.sendJson(res, 200, { tenant: publicTenant(switched.tenant), membership: saas.publicMembership(membership, switched.tenant) });
}

function apiInvitationsList(req, res, ctx) {
  const rows = core.db().invitations
    .filter((row) => row.organizationId === ctx.tenant.id)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt))
    .map(saas.publicInvitation);
  core.sendJson(res, 200, { invitations: rows });
}

async function apiInvitationCreate(req, res, ctx) {
  if (rejectImpersonated(res, ctx)) return;
  const ip = requestRateKey(req);
  if (!core.rateOk(`invite:${ip}`, 8, 8)) return core.sendJson(res, 429, { error: 'invitation rate limited', code: 'rate' });
  const b = ctx.body || {};
  const email = String(b.email || '').trim().toLowerCase();
  const role = String(b.role || 'viewer');
  const idempotencyKey = String(b.idempotencyKey || '').trim().slice(0, 120);
  if (!EMAIL_RE.test(email)) return core.sendJson(res, 422, { error: 'valid email required', code: 'bad_email' });
  if (!saas.ORG_ROLES.includes(role)) return core.sendJson(res, 422, { error: 'invalid organization role', code: 'bad_role' });
  if (!idempotencyKey) return core.sendJson(res, 422, { error: 'idempotencyKey required', code: 'idempotency_required' });
  let invite; let token; let duplicate = false;
  await core.mutate((d) => {
    const existing = d.invitations.find((row) => row.organizationId === ctx.tenant.id && row.idempotencyKey === idempotencyKey);
    if (existing) { invite = existing; duplicate = true; return; }
    const created = saas.createInvitation(d, ctx, email, role);
    invite = created.invite;
    token = created.token;
    invite.idempotencyKey = idempotencyKey;
    addAudit(d, ctx, 'invitation.created', 'invitation', invite.id, { email, role });
  });
  core.sendJson(res, duplicate ? 200 : 201, {
    invitation: saas.publicInvitation(invite),
    inviteUrl: token ? `/app.html#/accept-invite?token=${encodeURIComponent(token)}` : undefined,
    token: token || undefined,
    duplicate,
    note: token ? 'Invitation token is returned once. Store it securely.' : undefined,
  });
}

async function apiInvitationRevoke(req, res, ctx) {
  const inviteId = String((ctx.body || {}).invitationId || '');
  const invite = core.db().invitations.find((row) => row.id === inviteId && row.organizationId === ctx.tenant.id);
  if (!invite) return core.sendJson(res, 404, { error: 'invitation not found', code: 'not_found' });
  await core.mutate((d) => {
    const target = d.invitations.find((row) => row.id === inviteId);
    target.revokedAt = new Date().toISOString();
    saas.addInvitationAudit(target, 'invitation.revoked', ctx.user.id);
    addAudit(d, ctx, 'invitation.revoked', 'invitation', inviteId);
  });
  core.sendJson(res, 200, { ok: true });
}

async function apiInvitationAccept(req, res, ctx) {
  const token = String((ctx.body || {}).token || '');
  let result;
  await core.mutate((d) => { result = saas.acceptInvitation(d, token, ctx.user); });
  if (result.error) return core.sendJson(res, 400, { error: result.error, code: result.code });
  const tenant = core.db().tenants.find((row) => row.id === result.membership.organizationId);
  const switched = await saas.switchOrganization(req, ctx.user, tenant.id);
  await core.mutate((d) => addAudit(d, { ...ctx, tenant, membership: result.membership }, 'invitation.accepted', 'invitation', result.invite.id));
  core.sendJson(res, 200, { tenant: publicTenant(tenant), membership: saas.publicMembership(result.membership, tenant), switched: !switched.error });
}

function apiCredentialsList(req, res, ctx) {
  const rows = core.db().providerCredentials
    .filter((row) => row.organizationId === ctx.tenant.id)
    .map(saas.publicCredential);
  core.sendJson(res, 200, { credentials: rows });
}

async function apiCredentialSave(req, res, ctx) {
  if (rejectImpersonated(res, ctx)) return;
  const ip = requestRateKey(req);
  if (!core.rateOk(`cred:${ip}`, 12, 12)) return core.sendJson(res, 429, { error: 'credential rate limited', code: 'rate' });
  try {
    let row;
    await core.mutate((d) => {
      row = saas.upsertCredential(d, ctx, ctx.body || {});
      addAudit(d, ctx, 'credential.saved', 'credential', row.id, { provider: row.provider, layer: row.layer, maskedSuffix: row.maskedSuffix });
    });
    core.sendJson(res, 201, { credential: saas.publicCredential(row) });
  } catch (e) {
    core.sendJson(res, 422, { error: e.message, code: 'bad_credential' });
  }
}

async function apiCredentialDelete(req, res, ctx) {
  const credentialId = String((ctx.body || {}).credentialId || '');
  const row = core.db().providerCredentials.find((item) => item.id === credentialId && item.organizationId === ctx.tenant.id);
  if (!row) return core.sendJson(res, 404, { error: 'credential not found', code: 'not_found' });
  await core.mutate((d) => {
    d.providerCredentials = d.providerCredentials.filter((item) => item.id !== credentialId);
    addAudit(d, ctx, 'credential.deleted', 'credential', credentialId, { provider: row.provider, layer: row.layer });
  });
  core.sendJson(res, 200, { ok: true });
}

async function apiVoiceOverview(req, res, ctx) {
  const filters = saas.parseVoiceFilters(req.url);
  if (filters.demo && process.env.DEMO_ANALYTICS !== 'false') {
    await core.mutate((store) => saas.seedDemoVoiceData(store, ctx.tenant.id));
  }
  const data = saas.buildVoiceOverview(core.db(), ctx.tenant.id, filters);
  core.sendJson(res, 200, { data });
}

function apiVoiceFilters(req, res, ctx) {
  core.sendJson(res, 200, saas.voiceFilterOptions(core.db(), ctx.tenant.id));
}
...
async function apiAdminTenantCreate(req, res, ctx) {
  ...
  await core.mutate((store) => {
    ...
    tenant = {
      ...
      plan: 'studio', status: ownerEmail ? 'active' : 'onboarding', privacyMode: 'standard', parent_account_id: null,
    };
    ...
    if (ownerEmail) {
      ...
      saas.ensureMembership(store, user.id, tenant.id, 'owner');
    }
...
// GET /api/providers -> registry with workspace + server credential status.
function apiProviders(req, res, ctx) {
  core.sendJson(res, 200, {
    ...providers.describeProviders(ctx.tenant.id),
    credentialStatus: credentials.credentialStatus(ctx.tenant.id),
  });
}

function apiCredentialCatalog(req, res) {
  core.sendJson(res, 200, { catalog: credentials.publicCatalog() });
}

async function apiCredentialSaveCatalog(req, res, ctx) {
  if (rejectImpersonated(res, ctx)) return;
  const ip = requestRateKey(req);
  if (!core.rateOk(`cred:${ip}`, 12, 12)) return core.sendJson(res, 429, { error: 'credential rate limited', code: 'rate' });
  const b = ctx.body || {};
  const entry = credentials.catalogEntry(String(b.catalogId || ''));
  if (!entry) return core.sendJson(res, 422, { error: 'unknown catalog entry', code: 'bad_catalog' });
  try {
    const secret = credentials.buildSecretFromPayload(entry, b.fields || b);
    const layers = entry.layers || [entry.layer];
    const saved = [];
    await core.mutate((d) => {
      for (const layer of layers) {
        const row = saas.upsertCredential(d, ctx, {
          provider: entry.provider,
          layer,
          secret,
          label: entry.label,
          idempotencyKey: String(b.idempotencyKey || `${entry.id}:${layer}`).slice(0, 120),
        });
        saved.push(saas.publicCredential(row));
        addAudit(d, ctx, 'credential.saved', 'credential', row.id, { provider: row.provider, layer: row.layer, catalogId: entry.id });
      }
    });
    core.sendJson(res, 201, { credentials: saved });
  } catch (e) {
    core.sendJson(res, 422, { error: e.message, code: 'bad_credential' });
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (route.startsWith('/api/')) {
      ...
      if (route === '/api/providers' && req.method === 'GET') return core.requireAuth(req, res, apiProviders);
      if (route === '/api/credentials/catalog' && req.method === 'GET') return core.requireAuth(req, res, apiCredentialCatalog);
      ...
      if (req.method === 'GET') {
        ...
        if (route === '/api/telephony/transcripts') return saas.requireOrgRole('analyst')(req, res, apiTelephonyTranscriptsList);
        if (route === '/api/telephony/agent-analytics') return saas.requireOrgRole('analyst')(req, res, apiTelephonyAgentAnalytics);
        if (route === '/api/telephony/default-agent') return core.requireAuth(req, res, apiTelephonyDefaultAgentGet);
        if (route.startsWith('/api/telephony/transcripts/')) {
          const id = decodeURIComponent(route.slice('/api/telephony/transcripts/'.length));
          if (id.includes('/')) return core.sendJson(res, 404, { error: 'no such endpoint', code: 'not_found' });
          return core.requireAuth(req, res, (request, response, ctx) => apiTelephonyTranscriptGet(request, response, ctx, id));
        }
        ...
        if (route === '/api/members') return saas.requireOrgRole('admin')(req, res, apiMembers);
        if (route === '/api/audit') return saas.requireOrgRole('admin')(req, res, apiAudit);
        if (route === '/api/organizations') return core.requireAuth(req, res, apiOrganizations);
        if (route === '/api/organizations/invitations') return saas.requireOrgRole('admin')(req, res, apiInvitationsList);
        if (route === '/api/credentials') return saas.requireOrgRole('admin')(req, res, apiCredentialsList);
        if (route === '/api/voice/overview') return saas.requireOrgRole('analyst')(req, res, apiVoiceOverview);
        if (route === '/api/voice/filters') return saas.requireOrgRole('analyst')(req, res, apiVoiceFilters);
      ...
      if (route === '/api/telephony/dial') return saas.requireOrgRole('operator')(req, res, apiTelephonyDial, body);
      if (route === '/api/telephony/default-agent') return saas.requireOrgRole('admin')(req, res, apiTelephonyDefaultAgentSet, body);
      if (route.startsWith('/api/telephony/transcripts/') && route.endsWith('/refresh')) {
        const id = decodeURIComponent(route.slice('/api/telephony/transcripts/'.length, -'/refresh'.length));
        if (!id || id.includes('/')) return core.sendJson(res, 404, { error: 'no such endpoint', code: 'not_found' });
        return core.requireAuth(req, res, (request, response, ctx) => apiTelephonyTranscriptRefresh(request, response, ctx, id), body);
      }
      ...
      if (route === '/api/organizations/switch') return saas.requireOrgRole('viewer')(req, res, apiOrganizationSwitch, body);
      if (route === '/api/organizations/invitations') return saas.requireOrgRole('admin')(req, res, apiInvitationCreate, body);
      if (route === '/api/organizations/invitations/revoke') return saas.requireOrgRole('admin')(req, res, apiInvitationRevoke, body);
      if (route === '/api/organizations/invitations/accept') return core.requireAuth(req, res, apiInvitationAccept, body);
      if (route === '/api/credentials') return saas.requireOrgRole('admin')(req, res, apiCredentialSave, body);
      if (route === '/api/credentials/catalog') return saas.requireOrgRole('admin')(req, res, apiCredentialSaveCatalog, body);
      if (route === '/api/credentials/delete') return saas.requireOrgRole('admin')(req, res, apiCredentialDelete, body);
      if (route === '/api/tenant/update') return saas.requireOrgRole('owner')(req, res, apiTenantUpdate, body);
      if (route === '/api/tenant/pipeline') return saas.requireOrgRole('owner')(req, res, apiTenantPipeline, body);
      if (route === '/api/members/role') return saas.requireOrgRole('owner')(req, res, apiMemberRole, body);
...
boot().then(() => {
  server.listen(PORT, () => {
    ...
    console.log(`  Providers : deepgram ${flag('stt', 'deepgram')}  sarvam-stt ${flag('stt', 'sarvam')}  groq ${flag('llm', 'groq')}  sarvam-llm ${flag('llm', 'sarvam')}  rumik ${flag('tts', 'rumik')}  sarvam-tts ${flag('tts', 'sarvam')}  vobiz ${flag('telephony', 'vobiz')}  voicelink ${flag('telephony', 'voicelink')}\n`);
  });
}).catch((e) => {
