...
const ADMIN_EMAIL = 'agency.qa@vaani.ai';
const ADMIN_PASSWORD = 'Agency-QA-Only-2026!';
const CLIENT_EMAIL = 'owner.qa@vaani.ai';
...
test('agency OS APIs complete the lifecycle and preserve tenant isolation', { timeout: 30000 }, async (t) => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'vaani-agency-os-'));
  ...
  const child = spawn(process.execPath, ['server.js'], {
    ...
    env: {
      ...
      TEST_USER_TENANT: 'Vaani Agency QA',
  ...
  const fileMode = (await stat(dbFile)).mode & 0o777;
  assert.equal(fileMode, process.platform === 'win32' ? (fileMode === 0o666 ? 0o666 : 0o600) : 0o600);
});
