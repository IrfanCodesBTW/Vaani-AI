...
test('registry advertises implemented providers including Sarvam and VoiceLink', () => {
  resetEnv();
  const providers = loadProviders();
  const described = providers.describeProviders();
  assert.deepEqual(described.stt.map((row) => row.id), ['deepgram', 'sarvam']);
  assert.deepEqual(described.tts.map((row) => row.id), ['rumik', 'sarvam']);
  assert.deepEqual(described.llm.map((row) => row.id), ['groq', 'gemini', 'sarvam']);
  assert.deepEqual(described.telephony.map((row) => row.id), ['vobiz', 'voicelink']);
...
test('STT can be switched to Sarvam when configured', () => {
  resetEnv({ SARVAM_API_KEY: 'sarvam-test' });
  const providers = loadProviders();
  const choice = providers.resolveSelection('stt', { provider: 'sarvam', model: 'saarika:v2.5' });
  assert.equal(choice.provider, 'sarvam');
  assert.equal(choice.model, 'saarika:v2.5');
  assert.equal(choice.adapter.live, true);
});

test('model allowlists validate tenant-selected models', () => {
  resetEnv({
    GROQ_ALLOWED_MODELS: 'openai/gpt-oss-120b,openai/gpt-oss-20b',
