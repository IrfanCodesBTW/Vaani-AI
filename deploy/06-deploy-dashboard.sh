...
python3 - <<PY | rsh "cat > /opt/vaani-ai/.env"
import os
for k in [
...
rsh "docker rm -f vaani-ai 2>/dev/null; \
     docker run -d --name vaani-ai --restart always -p $PORT:$PORT \
       -v /opt/vaani-ai:/app -w /app node:20-alpine node server.js"
rsh "ufw allow $PORT/tcp || true"

sleep 3
rsh "docker logs vaani-ai --tail 10"
...
ok "Dashboard live at http://$VPS_IP:$PORT/app.html (demo@vaani.ai / vaaniai)"
